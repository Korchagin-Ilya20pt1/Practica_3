...
int main()
{
    ...
    strcpy(buffer,"Привет, мое имя-Илья!"); //копируем строку
    ...
    if (mySocket==-1){
        close(mySocket);
        Exception("Ошибка открытия сюжета", 11);
    }
    
    //свяязываем сюжет с адресом
